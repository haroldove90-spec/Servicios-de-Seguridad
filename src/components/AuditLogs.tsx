  ...
  return (
    <div id="analytics-and-logs-container" className="space-y-6">
      ...
      <div id="metrics-grid-board" className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div id="metric-tile-total" className="bg-[#0f172a] border border-[#1e293b] p-5 rounded-2xl shadow-2xl">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Lecturas Hechas</span>
            <span className="p-1.5 bg-slate-950 rounded-lg text-slate-400 border border-slate-800"><Activity className="w-3.5 h-3.5" /></span>
          </div>
        ...
        <div id="metric-tile-success" className="bg-[#0f172a] border border-[#1e293b] p-5 rounded-2xl shadow-2xl">
          <div className="flex items-center justify-between">
        ...
        <div id="metric-tile-failed" className="bg-[#0f172a] border border-[#1e293b] p-5 rounded-2xl shadow-2xl">
          <div className="flex items-center justify-between">
        ...
        <div id="metric-tile-presence" className="bg-[#0f172a] border border-[#1e293b] p-5 rounded-2xl shadow-2xl">
          <div className="flex items-center justify-between">
      ...
      <div id="logs-list-cabinet" className="bg-[#0f172a] rounded-2xl border border-[#1e293b] shadow-2xl overflow-hidden">
        
        {/* Table Filters Top Bar */}
        <div id="logs-filters-hub" className="p-5 border-b border-[#1e293b] space-y-4 md:space-y-0 md:flex md:items-center md:justify-between">
          <div className="relative flex-1 max-w-sm">
            ...
            <input
              ...
              type="text"
              placeholder="Buscar por visitante, cédula o guardia..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-10 pr-3 py-2 text-xs bg-slate-950 border border-slate-805 rounded-xl text-slate-100 placeholder-slate-500 focus:border-indigo-500 focus:outline-hidden"
            />
          </div>
          ...
          <div id="logs-filtering-controls" className="flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-1 bg-slate-950 px-2.5 py-1.5 rounded-lg border border-slate-800">
              ...
              >
                <option value="all" className="bg-slate-950">Filtro: Todo Movimiento</option>
                <option value={LogType.CHECK_IN} className="bg-slate-950">Check-in (Entradas)</option>
                <option value={LogType.CHECK_OUT} className="bg-slate-950">Check-out (Salidas)</option>
              </select>
            </div>
            ...
            <div className="flex items-center gap-1 bg-slate-950 px-2.5 py-1.5 rounded-lg border border-slate-800">
              ...
              <select
                ...
                className="text-[11px] bg-transparent font-semibold text-slate-200 focus:outline-hidden cursor-pointer"
              >
                <option value="all" className="bg-slate-950">Filtro: Todo Resultado</option>
                <option value={LogStatus.SUCCESS} className="bg-slate-950">Aprobados (✓)</option>
                <option value={LogStatus.ALREADY_USED} className="bg-slate-950">Intentos de Re-uso</option>
                <option value={LogStatus.EXPIRED_TOKEN} className="bg-slate-950">Pases Caducados</option>
                <option value={LogStatus.REVOKED_USER} className="bg-slate-950">Cuentas Suspendidas</option>
                <option value={LogStatus.OUTSIDE_SCHEDULE} className="bg-slate-950">Fuera de Horario</option>
              </select>
            </div>
            ...
            <button
              ...
              onClick={onRefresh}
              className="p-2 text-slate-400 bg-slate-950 hover:bg-slate-900 border border-slate-800 rounded-lg transition cursor-pointer"
            ...
            <button
              ...
              className="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs px-3.5 py-2 rounded-xl transition shadow-xs cursor-pointer"
            >
        ...
        <div id="reactive-logs-table-view" className="overflow-x-auto">
          {filteredLogs.length > 0 ? (
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-950 border-b border-[#1e293b] text-[10px] font-bold text-slate-400 uppercase tracking-wider">
              ...
              <tbody className="divide-y divide-slate-800/40 text-xs text-slate-200">
                {filteredLogs.map((log) => (
                  <tr key={log.id} id={`log-row-${log.id}`} className="hover:bg-slate-900/40 transition">
                    ...
                    <td className="py-3.5 px-4 text-indigo-305 font-mono whitespace-nowrap">
                      {log.documentId}
                    </td>
                    ...
                    <td className="py-3.5 px-4 whitespace-nowrap">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold border ${getStatusBadgeStyles(log.status)}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${log.status === LogStatus.SUCCESS ? 'bg-emerald-450 animate-pulse' : 'bg-rose-450'}`} />
                    ...
                    <td className="py-3.5 px-5 whitespace-nowrap text-right text-slate-400">
                      ...
                      <span className="block text-[9.5px] text-indigo-300 font-mono">{log.guardId}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <div id="no-filtered-logs-alert" className="text-center py-16 px-4 bg-slate-950 rounded-2xl border border-dashed border-slate-800 m-4">
        ...
        <div id="logs-footer-summary" className="p-4 bg-slate-950 border-t border-[#1e293b] flex items-center justify-between text-[11px] text-slate-400">
          <p>Mostrando <span className="font-semibold font-mono text-indigo-450">{filteredLogs.length}</span> registros de auditoría filtrados</p>
